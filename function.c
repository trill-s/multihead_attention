...
#include "util.h"
#include "args.h"
#include "/home/export/online3/swbxyh/fzz/multihead_attention/inputs.h"

/***************************
 * master function in master.c
 ***************************/
extern void SLAVE_FUN(funcRRR)(void* inputs);
extern void SLAVE_FUN(funcRCR)(void* inputs);
...
{   
    ...
    */
    ...
    if (inputs == NULL) inputs = create_empty_inputs();
    inputs->A=A;
    inputs->LDA=LDA;
    inputs->B=B;
    inputs->LDB=LDB;
...
void _local_gemm_rrr(const float* A, const int LDA, const float* B, const int LDB, float* C, const int LDC, int M, int N, int K)
{
    ...
    */
    //Inputs inputs = create_empty_inputs();
...
void _local_trans_head(float* src, float* dst, int B, int S, int D, int N)
{
    int pD = D/N;
...
void _local_trans_head_back(float* src, float* dst, int B, int S, int D, int N)
{
    int pD = D/N;
...
void _local_norm(float* buf, int len)
{
    double sum = 0.0f;
	for(int i = 0;i < len; i ++)
		sum += buf[i];
